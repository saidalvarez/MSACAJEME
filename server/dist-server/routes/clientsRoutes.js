...
router.get('/', async (req, res) => {
    try {
        logger_1.default.info('GET /api/clients');
        const query = `
      SELECT c.*,
    ...
    `;
        const [clients] = await base_de_datos_1.default.query(query);
        res.json(clients);
    }
    catch (error) {
        logger_1.default.error('Error GET /api/clients:', error);
        res.status(500).json({ error: 'Error interno del servidor' });
    }
});
router.post('/', async (req, res) => {
    try {
        logger_1.default.info('POST /api/clients - Body:', req.body);
        ...
        if (name) {
            ...
            while (await models_1.Cliente.findByPk(finalId)) {
                finalId = `${baseId}-${counter}`;
                counter++;
            }
            req.body.id = finalId;
        }
        const client = await models_1.Cliente.create(req.body);
        logger_1.default.info(`Client created: ${client.id}`);
        res.status(201).json(client);
    }
    catch (error) {
        logger_1.default.error('Error POST /api/clients:', error);
        res.status(500).json({ error: 'Error interno del servidor' });
    }
});
router.put('/:id', async (req, res) => {
    ...
    try {
        const { id } = req.params;
        const client = await models_1.Cliente.findByPk(id, { transaction: t });
        if (!client) {
            await t.rollback();
        ...
        if (newName !== oldName || newPhone !== oldPhone) {
            ...
            if (newId !== id) {
                let counter = 1;
                while (await models_1.Cliente.findByPk(newId, { transaction: t })) {
        ...
        if (newName !== oldName || newPhone !== oldPhone || newId !== id) {
            logger_1.default.info(`Cascading update for client ID ${id} -> ${newId}: ${oldName} -> ${newName}`);
            const whereClause = { client_id: id };
            // Update by client_id FIRST in all related tables
            await models_1.Ticket.update({ client_id: newId, client_name: newName, client_phone: newPhone }, { where: whereClause, transaction: t });
            await models_1.Sale.update({ client_id: newId, client_name: newName, client_phone: newPhone }, { where: whereClause, transaction: t });
            // LEGACY FALLBACK: Match by name (for legacy records missing client_id)
            const legacyWhere = { client_name: oldName, client_id: null };
            await models_1.Ticket.update({ client_id: newId, client_name: newName, client_phone: newPhone }, { where: legacyWhere, transaction: t });
            await models_1.Sale.update({ client_id: newId, client_name: newName, client_phone: newPhone }, { where: legacyWhere, transaction: t });
        }
        ...
        if (newId !== id) {
            ...
            const updatedClient = await models_1.Cliente.create(clientData, { transaction: t });
            await t.commit();
            ...
            await t.commit();
            res.json(await models_1.Cliente.findByPk(id));
        }
    }
    catch (error) {
        if (t)
...
router.delete('/:id', async (req, res) => {
    try {
        const client = await models_1.Cliente.findByPk(req.params.id);
        if (!client)
            return res.status(404).json({ error: 'Client not found' });
